...
// ---- weekly periods: a week runs Friday 00:00:00 -> next Friday 00:00:00 ----
...
function weekStart(when = Date.now()) {
  ...
  // fixed offset: shift into that zone, floor to Friday midnight, shift back
  const shifted = new Date(t.getTime() + TZ_OFFSET * 3600_000);
  const back = (shifted.getUTCDay() - 5 + 7) % 7;
  const floored = Date.UTC(
    shifted.getUTCFullYear(), shifted.getUTCMonth(), shifted.getUTCDate() - back, 0, 0, 0, 0
  );
  return floored - TZ_OFFSET * 3600_000;
}

function weekEnd(when = Date.now()) {
  return weekStart(when) + 7 * 86_400_000;
}

// Label like "Fri 18 Jul – Thu 24 Jul"
function weekLabel(when = Date.now()) {
  const s = new Date(weekStart(when));
  const e = new Date(weekEnd(when) - 86_400_000);
